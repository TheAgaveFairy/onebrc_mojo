...
def build_weather_station_name_list():
    ...
    """
    station_names = []
    with open(WEATHER_STATIONS_CSV, 'r') as file:  # EDITED PATH - PD
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    # handles small tests
    batch_size = 10000 if 10000 < num_rows_to_create else num_rows_to_create
    chunks = num_rows_to_create // batch_size
    if num_rows_to_create % batch_size != 0:
        print("Batch size doesn't evenly divide num_rows_to_create... skipping remainder")
    ...
    try:
        with open(OUTPUT_PATH, 'w') as file:
            ...
            for chunk in range(chunks):
                ...
                prepped_deviated_batch = '\n'.join([f"{station};{random.uniform(
                    coldest_temp, hottest_temp):.1f}" for station in batch])
